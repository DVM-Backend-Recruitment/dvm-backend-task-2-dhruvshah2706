...
from .forms import UserRegistrationForm, ProfileForm, CustomAuthenticationForm, UserUpdateForm
...
def regular_dashboard(request):
    ...
    if query:
        shows = shows.filter(
            ...
            Q(theater__name__icontains=query) |
            Q(screen__name__icontains=query)
        )
        print(shows)
        if not shows:
            is_empty = True
